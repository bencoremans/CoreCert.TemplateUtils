...
function Import-SerializedTemplate {
    ...
    process {
        # Apply in-memory substitutions — always parse XML to strip source OID (prevents CRYPT_E_EXISTS on re-import)
        [xml]$doc = $XmlString
        ...
        # --- OID reset (always) ---
        # The source template OID in the XML is registered in AD under the source template name.
        # Leaving it causes CRYPT_E_EXISTS on import (even after template deletion, the OID container
        # entry persists). Replace with a unique random OID so the COM interface accepts the import
        # and registers it under the new OID. The final OID stored in AD is generated by
        # CX509CertificateTemplateADWritable.Commit() — this injected OID is only used to pass
        # validation in CX509EnrollmentPolicyWebService.InitializeImport().
        $rnd = [System.Random]::new()
        $newOid = "1.3.6.1.4.1.311.21.8.$($rnd.Next(1000000,9999999)).$($rnd.Next(1000000,9999999))." +
                  "$($rnd.Next(1000000,9999999)).$($rnd.Next(1000000,9999999)).$($rnd.Next(1000000,9999999))." +
                  "$($rnd.Next(1000000,9999999)).$($rnd.Next(1000000,9999999))"
        $templateOidNodes = $doc.SelectNodes("//ep:oID/ep:value[starts-with(text(),'1.3.6.1.4.1.311.21.8')]", $ns)
        foreach ($node in $templateOidNodes) { $node.InnerText = $newOid }
        Write-Verbose "OID reset: replaced $($templateOidNodes.Count) template OID(s) with unique value to force new OID on import."
